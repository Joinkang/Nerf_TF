...
class NeRFMLP(tf.keras.Model):
    def __init__(self, n_layers: int = 8, dense_units: int = 256, skip_layer=4, initializer='glorot_uniform', **kwargs):
        ...
        self.mlp_layers = [
            tf.keras.layers.Dense(
                units=dense_units, activation=tf.keras.layers.LeakyReLU(alpha=0.01), name=f"layer_{i}", kernel_initializer=initializer)
            for i in range(n_layers)
        ]

        self.sigma = tf.keras.layers.Dense(
            units=1, activation=tf.keras.layers.LeakyReLU(alpha=0.01), name="sigma", kernel_initializer=initializer)
